...
const ModuleSidebar = ({ trans, menusConfig }: { trans: any; menusConfig: any }) => {
  ...
  const toggleSubMenu = (index: number) => {
    ...
    if (menusList[index].child) {
      setCurrentSubMenu(menusList[index].child);
      setSubmenu(false);
      setCollapsed(false);
      if (!isDesktop) {
        setMenuOverlay(true);
      ...
      if (!isDesktop) {
        ...
        if (isLocationMatch(menusList[index].title, locationName)) {
          setSubmenu(false);
        }
      }
    }
  ...
  const getMenuTitle = () => {
    if (activeIndex !== null) {
      return menusList[activeIndex].title;
    }
    return "";
  };
  ...
  return (
    <>
      <div className="main-sidebar  pointer-events-none fixed start-0 top-0 z-[60] flex h-full xl:z-10 print:hidden">
        <div
          className={cn("border-default-200  dark:border-default-300 pointer-events-auto relative z-20 flex h-full w-[72px] flex-col border-r border-dashed   bg-card transition-all duration-300", {
        ...
        >
          <div className=" pt-4 ">
            <Link href="/dashboard">
              <Image src={SiteLogo} alt="SiteLogo" className=" mx-auto text-primary h-8 w-8" />
            </Link>
          </div>
          {/* end logo */}
          <ScrollArea className=" pt-6 grow ">
            {menusList.map((item, i) => (
              <div key={i} onClick={() => toggleSubMenu(i)} className=" mb-3 last:mb-0">
                <SingleIconMenu index={i} activeIndex={activeIndex} item={item} locationName={locationName} trans={trans} />
              </div>
            ))}
          </ScrollArea>
          <FooterMenu />
        ...
        <div
          className={cn("border-default-200 pointer-events-auto relative z-10 flex flex-col h-full w-[248px] border-r  bg-card   transition-all duration-300", {
              ...
              <ul>
                {currentSubMenu?.map((childItem, j) => (
                  <li key={j} className="mb-1.5 last:mb-0">
                    <MenuItem trans={trans} childItem={childItem} toggleNested={toggleNested} index={j} nestedIndex={nestedIndex} locationName={locationName} />
                    <NestedMenus index={j} nestedIndex={nestedIndex} nestedMenus={childItem.nested} locationName={locationName} toggleMulti={toggleMultiNested} multiIndex={multiNestedIndex} trans={trans} />
                  </li>
                ))}
              </ul>
            </div>
