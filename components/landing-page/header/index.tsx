        ...
        <div className={scroll ? "bg-card/50 dark:bg-card/70 backdrop-blur-lg z-50 shadow-sm fixed top-0 left-0 w-full py-3" : "fixed top-0 left-0 w-full py-3"}>
          <nav className="container flex justify-between relative z-50">
            <Link href="/" className="flex items-center gap-1">
              <Image src={SiteLogo} alt="SiteLogo" className="h-8 w-8 text-primary" />
              <span className="text-primary-500 font-medium text-xl">TFAC BackOffice</span>
            </Link>
                  ...
                  {menus?.map((item, i) =>
                    item.child ? (
                      <div className="space-y-1.5">
                        <div className=" flex items-center justify-between pr-4   cursor-pointer group" onClick={() => setShow(!show)}>
