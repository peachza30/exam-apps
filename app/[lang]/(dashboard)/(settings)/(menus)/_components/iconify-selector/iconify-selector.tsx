...
const IconSelectorWithAPI = ({ onIconSelect, selectedIcon = "" }) => {
  ...
  const fetchCollectionIcons = useCallback(async (collectionId, limit = 50) => {
    ...
    try {
      ...
      if (data.icons) {
        const iconList = Object.keys(data.icons)
          .slice(0, limit)
          .map(iconName => ({
            name: iconName,
  ...
  return (
    <>
      ...
      <div className="relative">
        <div className="flex items-center border border-gray-300 rounded-lg focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-transparent px-3 py-2">
          <button type="button" onClick={() => setIsOpen(!isOpen)} className="mr-2 p-2 hover:bg-gray-100 rounded-md transition-colors">
            {selectedIcon ? <Icon icon={selectedIcon} className="w-5 h-5 text-gray-600" /> : <Icon icon="mdi:chevron-down" className="w-5 h-5 text-gray-400" />}
          </button>
          <input type="text" onClick={() => setIsOpen(!isOpen)} value={selectedIcon} onChange={e => onIconSelect(e.target.value)} placeholder="Select an icon or type icon name..." className="w-full focus:outline-none" />
        </div>
      </div>

      {/* Dropdown */}
      {isOpen && (
        <div className="relative z-50 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-96 overflow-hidden">
          {/* Search */}
          <div className="p-3 border-b border-gray-200">
            <input type="text" value={searchTerm} onChange={e => handleSearch(e.target.value)} placeholder="Search all icons..." className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500" />
            {searchMode && <div className="mt-2 text-sm text-blue-600">🔍 Searching across all {collections.length} collections...</div>}
          </div>

          {/* Collection Selector (only show when not searching) */}
          {!searchMode && (
            <div className="p-3 border-b border-gray-200">
              <label className="block text-sm font-medium text-gray-700 mb-2">Icon Collection ({collections.length} available):</label>
              <select value={selectedCollection} onChange={e => handleCollectionChange(e.target.value)} className="w-full px-3 py-1 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 text-sm">
          ...
          <div className="max-h-64 overflow-y-auto">
            {loading ? (
              <div className="flex items-center justify-center py-8">
                ...
                <span className="ml-2 text-gray-600">{searchMode ? "Searching icons..." : "Loading collection..."}</span>
              </div>
            ) : icons.length > 0 ? (
              <div>
                <div className="px-3 py-2 text-sm text-gray-600 bg-gray-50">{searchMode ? `Found ${icons.length} icons` : `Showing ${icons.length} icons from ${selectedCollection}`}</div>
                <div className="grid grid-cols-6 gap-1 p-3">
                  {icons.map(icon => (
                    <button
                      ...
                      title={icon.fullName}
                    >
                      <Icon icon={icon.fullName} className="w-6 h-6 text-gray-700 group-hover:text-blue-600 transition-colors" />
                      <span className="text-xs text-gray-500 mt-1 truncate max-w-full group-hover:text-blue-600">{icon.name}</span>
                    </button>
                  ))}
                </div>
              </div>
            ) : (
              <div className="text-center py-8 text-gray-500">{searchMode ? "No icons found for this search" : "No icons available"}</div>
            )}
          </div>
        </div>
      )}
  ...
  return (
    <div className="mt-8 p-4 bg-gray-50 rounded-lg">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-medium text-gray-900">Available Icon Collections</h3>
        <button onClick={fetchCollections} disabled={loading} className="px-3 py-1 bg-blue-500 text-white rounded-md text-sm hover:bg-blue-600 disabled:opacity-50">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-64 overflow-y-auto">
          {collections.map(collection => (
            <div key={collection.id} className="p-3 bg-white rounded-md border hover:shadow-md transition-shadow cursor-pointer" onClick={() => setSelectedCollection(selectedCollection?.id === collection.id ? null : collection)}>
              <div className="flex items-center justify-between">
                ...
                <div className="flex space-x-1">
                  {collection.samples?.slice(0, 3).map((sample, idx) => (
                    <Icon key={idx} icon={`${collection.id}:${sample}`} className="w-4 h-4 text-gray-600" />
                  ))}
                </div>
              </div>
              {selectedCollection?.id === collection.id && (
                <div className="mt-3 pt-3 border-t text-xs text-gray-600">
                  <p>
                    <strong>Author:</strong> {collection.author?.name || "Unknown"}
                  </p>
                  <p>
                    <strong>License:</strong> {collection.license?.title || "Unknown"}
                  </p>
                  <p>
                    <strong>Category:</strong> {collection.category || "General"}
                  </p>
                </div>
              )}
            </div>
...
export default function IconifySelector() {
  const { iconName, setIconName, mode } = useMenuStore();
  ...
  useEffect(() => {
    if (mode === "edit") {
      ...
      setIconName("material-symbols:home");
    }
  }, [selectedIcon]);
  ...
  return (
    <div className="">
      <div className="">
        <div className="">
          <div className="mb-6">
            <IconSelectorWithAPI selectedIcon={iconName} onIconSelect={setIconName} />
          </div>

          {/* Preview Selected Icon */}
          {/* {selectedIcon && (
            <div className="mt-6 p-4 bg-gray-50 rounded-lg">
              <h3 className="text-lg font-medium text-gray-900 mb-3">Selected Icon Preview:</h3>
